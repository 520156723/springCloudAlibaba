package per.hqd.usercenter.auth;
...
public class AuthAspect {
    ...
    @Around("@annotation(per.hqd.usercenter.auth.CheckLogin)")
    ...
    @Around("@annotation(per.hqd.usercenter.auth.CheckAuthorization)")
    public Object checkAuthorization(ProceedingJoinPoint point) throws Throwable {
        try {
            ...
            if (!StringUtils.equals(role, annotation.value())) {
                throw new SecurityException("用户角色" + role + "无权访问！");
            }
        } catch (Throwable throwable) {
